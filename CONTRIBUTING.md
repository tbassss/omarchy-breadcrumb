...
The first two implementation slices (issues #3 and #4) add a native plugin layout and a Python 3 stdlib SQLite store. Run `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tests -v`, `python3 -m py_compile bin/breadcrumb-store tests/*.py`, Markdown link checks, issue-template frontmatter, and `git diff --check`. Store tests cover persistence, CAS, activity isolation, archive, history restore, faults, permissions, and safe links. Plugin tests are source contracts, not Omarchy runtime evidence. Isolated native Compact/Expanded save/reopen/Panel-recreate plus create A/B, switch, archive access, history restore, compact picker `selectCurrent` resync, and dirty-create refusal is `tests/native/` on the-cave (component test, not live desktop acceptance). See `docs/IMPLEMENTATION.md`.
