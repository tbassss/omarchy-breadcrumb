#!/usr/bin/env bash
# Isolated Cave native unarchive-journey validation.
...
HARNESS_QML="${HARNESS_QML:-unarchive-shell.qml}"
...
WORKDIR=$(mktemp -d /tmp/breadcrumb-native-unarchive-XXXX)
...
print("compactHadUnarchive=" + str(data.get("compactHadUnarchive")))
print("compactHadDelete=" + str(data.get("compactHadDelete")))
print("unarchivePreservedSelection=" + str(data.get("unarchivePreservedSelection")))
print("unarchivePreservedRevision=" + str(data.get("unarchivePreservedRevision")))
print("unarchivePreservedDraft=" + str(data.get("unarchivePreservedDraft")))
print("deleteHiddenAfterUnarchive=" + str(data.get("deleteHiddenAfterUnarchive")))
print("confirmClearedOnUnarchive=" + str(data.get("confirmClearedOnUnarchive")))
print("keyboardUnarchivePresent=" + str(data.get("keyboardUnarchivePresent")))
print("keyboardUnarchiveActivated=" + str(data.get("keyboardUnarchiveActivated")))
print("neighborKept=" + str(data.get("neighborKept")))
print("findings=" + json.dumps(data.get("findings")))
if not ok:
    print("ui_error=" + str(data.get("error", "")))
    sys.exit(1)
if data.get("compactHadUnarchive"):
    print("compact showed Unarchive")
    sys.exit(1)
if data.get("compactHadDelete"):
    print("compact showed Delete permanently request")
    sys.exit(1)
if not data.get("unarchivePreservedSelection"):
    print("unarchive did not preserve selection")
    sys.exit(1)
if not data.get("unarchivePreservedRevision"):
    print("unarchive appended a checkpoint")
    sys.exit(1)
if not data.get("unarchivePreservedDraft"):
    print("unarchive dropped in-memory draft")
    sys.exit(1)
if not data.get("deleteHiddenAfterUnarchive"):
    print("delete remained after unarchive")
    sys.exit(1)
if not data.get("confirmClearedOnUnarchive"):
    print("pending delete confirm survived unarchive")
    sys.exit(1)
if not data.get("keyboardUnarchivePresent"):
    print("keyboard Unarchive missing")
    sys.exit(1)
if not data.get("keyboardUnarchiveActivated"):
    print("keyboard Unarchive did not activate")
    sys.exit(1)
if not data.get("neighborKept"):
    print("neighbor activity was mutated")
    sys.exit(1)
print("native unarchive asserts ok")
PY
