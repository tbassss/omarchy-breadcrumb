...
class TestPluginContract(unittest.TestCase):
    ...
    def test_panel_has_compact_expanded_and_honest_states(self) -> None:
        ...
        self.assertIn('objectName: "saveCheckpointButton"', qml)
        self.assertIn('objectName: "addLinkButton"', qml)
        self.assertIn('objectName: "restoreCheckpointButton"', qml)
        self.assertIn('objectName: "openLinkButton"', qml)
        self.assertIn("collectLabeledButtons", qml)
        self.assertIn('"Restore"', qml)
        self.assertIn("openProcRunning", qml)
    ...
    def test_native_harness_asserts_recreate_editor_and_draft_preserve(self) -> None:
        ...
        self.assertIn("BREADCRUMB_FAKE_OPEN_HANG", runner_text)
        self.assertIn("overlay-files.sha256", runner_text)
        ...
        self.assertIn("editor j did not modify text", qml)
        self.assertIn("dropdown popupOpen did not open", qml)
        self.assertIn("keyboard did not reach Save", qml)
        self.assertIn("Save was not in visible viewport after traversal", qml)
        self.assertIn("keyboard did not reach Links Open", qml)
        self.assertIn("History Restore was not in visible viewport after traversal", qml)
        self.assertIn("hang launcher env missing", qml)
        self.assertIn("hang did not wait production 8s timeout", qml)
        self.assertIn("hang child was not reaped", qml)
        self.assertIn("BREADCRUMB_FAKE_OPEN_HANG", qml)
